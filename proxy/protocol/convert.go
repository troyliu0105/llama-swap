...
// responseConverters maps (from, to) format pairs to response body conversion functions.
// "from" is the upstream format (what we received), "to" is the client format (what we send back).
var responseConverters = map[convertKey]func([]byte) ([]byte, error){
	{FormatOpenAI, FormatResponses}:    convertOpenAIResponseToResponses,
	{FormatResponses, FormatOpenAI}:    convertResponsesResponseToOpenAI,
	{FormatOpenAI, FormatAnthropic}:    convertOpenAIResponseToAnthropic,
	{FormatAnthropic, FormatOpenAI}:    convertAnthropicResponseToOpenAI,
	{FormatResponses, FormatAnthropic}: convertResponsesResponseToAnthropic,
	{FormatAnthropic, FormatResponses}: convertAnthropicResponseToResponses,
}
...
func (c *Converter) ConvertResponse(body []byte) ([]byte, error) {
	// Responses flow upstream→client, so swap the direction.
	convertFn, ok := responseConverters[convertKey{c.To, c.From}]
	if !ok {
		return nil, fmt.Errorf("unsupported response conversion: %s → %s", c.To, c.From)
	}
	return convertFn(body)
}
