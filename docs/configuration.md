...
# - each key is a non-empty string without spaces
# - supports list format (backward compatible) and dict format (with optional model restrictions)
#
# List format - all keys have unrestricted access:
apiKeys:
  - "sk-hunter2"
...
# Dict format with model restrictions - restrict which models each key can access:
# apiKeys:
#   "sk-restricted":
#     models: ["gpt-5.2", "claude-sonnet-4"]  # only these models
#   "sk-full-access":                          # no models list = unrestricted
#   "sk-no-models":
#     models: []                               # empty list = no model access
